...
// Puntatore globale per l'arco animato
static lv_obj_t *state_arc = NULL;
#define ARC_LINE_WIDTH 5
#define ARC_SPAN 45

// Definizione dei colori per l'arco
...
// Funzioni forward: le manteniamo tutte, senza rimuovere riferimenti o semplificazioni
...
//------------------------------------------------------------------------------
// Callback che aggiorna l'opacità della label durante fade in/out
//------------------------------------------------------------------------------
static void anim_opacity_cb(void *var, int32_t v)
{
    // Usiamo la label passata come var, evitando di accedere direttamente a 'state_label'
    if (!var) return;
...
//------------------------------------------------------------------------------
// Avvia l'animazione di fade-in (0->255)
static void fade_in_anim_start(lv_obj_t *lbl)
{
    if (!lbl) return;

    // Rimuoviamo eventuali animazioni precedenti su 'lbl'
...
//------------------------------------------------------------------------------
// Callback a fine fade-out
// Qui aggiorniamo il testo, poi avviamo il fade-in
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
// Avvia l'animazione di fade-out (255->0) con callback
//------------------------------------------------------------------------------
static void fade_out_anim_start(lv_obj_t *lbl)
{
    if (!lbl) return;

    // Rimuove eventuali anim in corso su lbl
    lv_anim_del(lbl, NULL);
...
{
    // Proteggiamo l'intera funzione con un lock
    _lock_acquire(&lvgl_api_lock);

    // Aggiorna new_text in base allo stato
    switch (state) {
    ...
    // Se la label principale non esiste, creala
    if (state_label == NULL) {
        ...
        // number_label in font più grande
        ...
        // Fade in al primo avvio
        fade_in_anim_start(state_label);
    }
    else {
        // Avviamo un fade out per cambiare testo
        fade_out_anim_start(state_label);
    }

    // Se siamo in SHOW_PRACTICES, mostriamo number_label
    if (state == DISPLAY_STATE_SHOW_PRACTICES) {
        ...
        // Nascondi number_label in altri stati
        lv_obj_add_flag(number_label, LV_OBJ_FLAG_HIDDEN);
    }

    // Gestione arco animato (stati in cui serve)
    ...
    if (arc_needed) {
        if (state_arc == NULL) {
            ...
            // Annulliamo eventuali anim su arc
        ...
        // Se non serve, rimuoviamo l'arco
        if (state_arc != NULL) {
